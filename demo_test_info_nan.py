# -*- coding: utf-8 -*-
# @Time    : 2019-05-29 21:58
# @Author  : edward
# @File    : demo_test_info_nan.py
# @Software: PyCharm
import argparse
import logging
import os
...
from utils import check_exists, init_logging, info_vid_pre_progress, sep_cat_info_vid_transforms

logger = logging.getLogger(__name__)


def main(load_root, data_root, num_frame, num_attn, moda, epoch):
    print(load_root)
    load_path = os.path.join(load_root, 'demo_arcface_{}_nan_model_{:0>4d}.pth'.format(moda, epoch))
    print(load_path)
    assert check_exists(load_path)

    dataset = IQiYiVidDataset(data_root, 'test', moda,
    ...
    model = ArcFaceNanModel(512 + 2 + 1 + 3, 10034 + 1, num_attn=num_attn)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load_root', default=None, type=str)
    ...
    parser.add_argument('--meta_info', default=None, type=str, help='')
    ...
    all_outputs, all_video_names = main(args.load_root, args.data_root, args.num_frame, args.num_attn, args.moda, args.epoch)
