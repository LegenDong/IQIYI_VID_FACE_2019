...
__all__ = ['init_logging', 'check_exists', 'load_train_gt_from_txt', 'load_val_gt_from_txt', 'load_face_from_pickle',
           ...
           'sep_cat_qds_select_face_audio_scene_transforms', 'default_face_audio_scene_pre_progress']
...
def sep_cat_qds_mixup_vid_transforms(vid_info, modes, num_frame=15, norm_value=100., mixup_rate=0.5, **kwargs):
    ...
    for mode in modes:
        ...
        balance_weight = np.random.beta(mixup_rate, mixup_rate)
        feats = balance_weight * feats1 + (1 - balance_weight) * feats2
...
def default_face_audio_scene_pre_progress(face_feat_infos, audio_feat_infos, scene_feat_infos, gt_infos, **kwargs):
    audio_feat_info_dict = {}
    for audio_feat_info in audio_feat_infos:
        audio_feat_info_dict[audio_feat_info['video_name']] = audio_feat_info['feat']
    vid_infos = {}
...
def sep_cat_qds_select_face_audio_scene_transforms(vid_info, face_mask=None, audio_mask=None, scene_mask=None,
                                                   num_frame=15, norm_value=100., **kwargs):
    result = []
    face_frame_infos = vid_info['face']
    if len(face_frame_infos) < num_frame:
        frames_infos = np.random.choice(face_frame_infos, num_frame, replace=True)
