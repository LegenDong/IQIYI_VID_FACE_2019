...
# @Author  : LegenDong
# @User    : legendong
# @File    : demo_train_face_scene_multi_view.py
# @Software: PyCharm
import argparse
import os
import pickle
...
from models import FocalLoss, ArcMarginProduct, ArcFaceSceneModel
...
def main(args):
    ...
    model = ArcFaceSceneModel(len(face_mask_index) + 2, len(scene_mask_index), args.num_classes, )
    ...
    save_model(model, args.save_dir, 'demo_arcface_face+scene_nan_{}_model'.format(model_id), args.epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scene_root', default='./scene_feat', type=str,
                        help='path to load data (default: /data/materials/)')
    parser.add_argument('--save_dir', default='./checkpoints/multi_view_face_scene', type=str,
                        help='path to save model (default: ./checkpoints/multi_view/multi_view_face_scene)')
